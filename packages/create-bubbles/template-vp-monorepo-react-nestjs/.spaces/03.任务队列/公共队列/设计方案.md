...
> 状态：历史实施方案；公共队列基础能力已进入当前源码，本文保留设计依据和接入说明，不作为当前验收报告  
> 基线日期：2026-09-03  
> 当前事实复核：2026-09-17  
...
当前源码已经包含 BullMQ 依赖、队列配置、`TaskQueueModule`、`TaskQueueService`、`TaskQueueProcessor`、Worker 启停注册器及相关测试文件。上传清理仍是独立设计，不能因公共队列已落地而视为已经实现。
...
上传清理只作为后续附加项，详细说明见 [上传清理设计方案](../../02.上传与文件/上传清理/设计方案.md)，当前源码尚未实现该业务。

---

## 3. 历史项目基线（2026-09-03）

下表记录公共队列实施前的源码状态，仅用于理解当时为何需要本方案；判断当前能力时应以源码和本文开头的当前事实复核为准。
...
以下复选框保留原实施方案的验收范围，不表示当前代码尚未完成；当前结论应以专项测试和交付记录为准。
...
详细方案已经拆分到独立文件：[上传清理设计方案](../../02.上传与文件/上传清理/设计方案.md)。该文件说明如何复用公共队列，但相关清理业务尚未进入当前源码。
