...
打开新生成的 `apps/server/drizzle/*.sql`，确认包含新增字段、表和索引，没有意外删除旧字段。当前仓库完整迁移链受企业层级残留 `0003` 和未确认邀请实现 `0004` 阻断，不得执行 `db:migrate`、手工跳号或使用 `db:push` 绕过。两项阻断解除并完成隔离验证后，按[数据库迁移指南](../../05.运行维护/数据库迁移/操作指南.md)执行经审阅的迁移，再运行：

```powershell
vp run --filter server db:studio
```

届时 Studio 应能看到新结构。`db:generate` 只生成 SQL，不会修改数据库；只改 TypeScript 同样不会自动建表。
...
全仓迁移阻断解除并完成隔离验证后，上线已有服务时才先执行经审阅的数据库迁移，保持清理预览模式，部署完所有支持分片租约的新 API/Worker，排空旧实例及旧在途上传，再启用实际清理。新旧版本混跑时，旧 API 不登记租约，后台无法知道它正在上传，不能提前打开实际清理。
...
| 日期       | 变更摘要                                                                                   | 影响范围             |
| ---------- | ------------------------------------------------------------------------------------------ | -------------------- |
| 2026-09-18 | 标明当前仓库完整迁移链受 `0003`／`0004` 阻断，上传清理迁移在阻断解除前只生成和审阅、不执行 | 迁移、联调与上线顺序 |
