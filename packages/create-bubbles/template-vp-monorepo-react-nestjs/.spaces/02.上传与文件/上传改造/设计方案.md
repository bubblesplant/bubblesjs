...
> 状态：讨论稿，尚未实施；第 2 节保留并校正 2026-09-02 的历史基线  
> 基线日期：2026-09-02；当前事实复核：2026-09-17  
...
- 现有分片上传接口与新增归属字段、文件记录及完成响应的兼容调整。
...
以下内容已按 2026-09-17 源码复核，不代表目标状态。原方案中“没有 company、project、membership 模型”以及“上传模块尚未接通”的描述已经过期，现按当前事实修正。
...
| 公司/项目模型 | 已有 companies、projects、company_members、project_members | 上传模块尚未接入这些模型，仍未校验 companyId、projectId |

当前上传模块已经具备 Controller 五个路由、UploadModule 注册、`initiate()` 主流程和独立会话参数 DTO。仍需在正式改造中处理：
...
### 阶段 1：接入已有 company/project 权限事实来源

- 复用现有 companies、projects、company_members、project_members 模型及访问控制能力。
- 从 projectId 反查并校验 companyId，不能只信任客户端参数。
- 将现有公司/项目成员资格与上传权限检查接入上传初始化、查询、分片、完成和取消流程。

现有模型并不等于上传模块已经完成归属校验。在本阶段完成前，不应仅靠新增两个 DTO 字段就把 companyId、projectId 写入数据库。

### 阶段 2：收尾上传模块基线

- 保留并回归验证当前已有的五个 Controller 路由、UploadModule、`initiate()` 主流程和独立会话参数 DTO。
- 统一 abort 响应。
- 修正相关文件名拼写及引用。
- 添加当前行为回归测试。
